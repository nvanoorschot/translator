;(function() {
  ...
  Translator.prototype.addTable = function(locale, translations) {
    ...
    for (let prop in translations) {
      ...
      if (Object.getOwnPropertyNames(translations[prop].defaults).length > 0) {
        for (let option in translations[prop].defaults) {
          defaults.innerText += translations[prop].defaults[option] + '\n'
        }
      }
