;(function() {
  ...
  // finish their requests, because during those other ajax requests additional translations might
  // get triggered that otherwise would be missed.
  d.addEventListener("DOMContentLoaded", function() {
    var elements = d.getElementsByClassName("translator")

    if (elements !== undefined) {
      var missing_translation = false
      ...
      setTimeout(function(){
        t.getTranslations(function(data) {
          for (var locale in data) {
            if (data.hasOwnProperty(locale) && missing_translation === false) {
              for (var key in data[locale]) {
                if (data[locale].hasOwnProperty(key) && missing_translation === false) {
          ...
          for (var i = 0; i < elements.length; i++) {
            if (missing_translation) { elements[i].style.color = "yellow" }
          }
        })
      }, 2000)
    }
  ...
  Translator.prototype.save = function() {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      var disable = value === element.value || (value === null && element.value === "")
